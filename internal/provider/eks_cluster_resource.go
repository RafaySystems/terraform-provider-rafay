...
func (r *eksClusterResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	for _, ng := range vNodeGroupsList {
		if !ng.Ssh.IsNull() && !ng.Ssh.IsUnknown() {
			...
			if len(vSshList) > 0 {
				if !vSshList[0].EnableSsm.IsNull() && !vSshList[0].EnableSsm.ValueBool() {
					resp.Diagnostics.AddError(
						"Invalid Configuration",
						"SSM agent is now built into EKS AMIs and cannot be disabled.",
					)
				}
			}
		}
	...
	for _, mngNg := range vMngNodeGroupsList {
		if !mngNg.Ssh4.IsNull() && !mngNg.Ssh4.IsUnknown() {
			vSshList := make([]resource_eks_cluster.Ssh4Value, 0, len(mngNg.Ssh4.Elements()))
	...
	for _, ngMap := range vngMap {
		if !ngMap.Ssh6.IsNull() && !ngMap.Ssh6.IsUnknown() {
			...
			sshVal := tfSshValue.(resource_eks_cluster.Ssh6Value)
			if !sshVal.EnableSsm.IsNull() && !sshVal.EnableSsm.ValueBool() {
				resp.Diagnostics.AddError(
					"Invalid Configuration",
					"SSM agent is now built into EKS AMIs and cannot be disabled.",
				)
	...
	for _, mngNgMap := range vmngNgMap {
		if !mngNgMap.Ssh5.IsNull() && !mngNgMap.Ssh5.IsUnknown() {
			...
			sshVal := tfSshValue.(resource_eks_cluster.Ssh5Value)
			if !sshVal.EnableSsm.IsNull() && !sshVal.EnableSsm.ValueBool() {
				resp.Diagnostics.AddError(
					"Invalid Configuration",
					"SSM agent is now built into EKS AMIs and cannot be disabled.",
...
func (r *eksClusterResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	tm := "60m"
	if !data.Timeouts.IsNull() && !data.Timeouts.Create.IsNull() && !data.Timeouts.Create.IsUnknown() {
		tm = data.Timeouts.Create.ValueString()
	}
	timeouts, err := time.ParseDuration(tm)
	if err != nil {
		resp.Diagnostics.Append(diag.NewErrorDiagnostic(
			"Timeout Cannot Be Parsed",
			fmt.Sprintf("timeout for %q cannot be parsed, %s", timeouts, err),
		))
		return
	}
...
LOOP:
	for {
		...
		case <-ctx.Done():
			...
			return
		case <-ticker.C:
			log.Printf("Cluster operation not completed for cluster: %s and project: %s. Waiting 60 seconds more for cluster to complete the operation.", clusterName, projectName)
			...
			if strings.Contains(sres.Status, "STATUS_COMPLETE") {
				...
				if conditionsFailure {
					log.Printf("blueprint sync failed for cluster: %s and project: %s", clusterName, projectName)
					resp.Diagnostics.AddError("Client Error", fmt.Sprintf("blueprint sync failed for cluster: %s and project: %s", clusterName, projectName))
					return
				} else if clusterReadiness {
					log.Printf("Cluster operation completed for cluster: %s and project: %s", clusterName, projectName)
					break LOOP
				} else {
...
LOOP:
	for {
		select {
		case <-ctx.Done():
			log.Printf("Cluster Deletion for cluster: %s and project: %s got timeout out.", clusterName, projectName)
			resp.Diagnostics.AddError("Client Error", fmt.Sprintf("cluster deletion for cluster: %s and project: %s got timeout out", clusterName, projectName))
			return
		case <-ticker.C:
			...
			log.Printf("Cluster Deletion is in progress for cluster: %s and project: %s. Waiting 60 seconds more for operation to complete.", clusterName, projectName)
		}
	}
	log.Printf("Cluster Deletion completes for cluster: %s and project: %s", clusterName, projectName)
