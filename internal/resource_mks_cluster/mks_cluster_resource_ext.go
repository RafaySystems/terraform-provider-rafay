...
func (v NetworkValue) ToHub(ctx context.Context) (*infrapb.MksClusterNetworking, diag.Diagnostics) {
	...
	if !v.DnsRecursors.IsNull() && !v.DnsRecursors.IsUnknown() {
		for _, val := range v.DnsRecursors.Elements() {
			hub.DnsRecursors = append(hub.DnsRecursors, getStringValue(val.(types.String)))
		}
	}
...
func (v NetworkValue) FromHub(ctx context.Context, hub *infrapb.MksClusterNetworking) (basetypes.ObjectValue, diag.Diagnostics) {
	...
	if len(hub.DnsRecursors) > 0 {
		elements := make(map[string]attr.Value, len(hub.DnsRecursors))
		for _, r := range hub.DnsRecursors {
			elements[r] = types.StringValue(r)
		}
		v.DnsRecursors, d = types.MapValue(types.StringType, elements)
		diags = append(diags, d...)
	} else {
		v.DnsRecursors = types.MapNull(types.StringType)
	}
