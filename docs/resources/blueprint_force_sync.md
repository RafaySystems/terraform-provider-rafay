...
page_title: "rafay_blueprint_force_sync Resource - terraform-provider-rafay"
subcategory: ""
description: |-
  Triggers a blueprint sync on a cluster, optionally assigning a blueprint name/version first.
---

# rafay_blueprint_force_sync (Resource)
...
resource "rafay_blueprint_force_sync" "cluster5" {
...
- `addons` (List of String, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Subset of addons to sync. Only valid with force_sync=true. Never stored in state.
