...
```terraform
resource "rafay_driver" "driver-example" {
  metadata {
    name    = var.name
    project = var.project
  }
...
### Nested Schema for `spec.config.container.working_dir_path`
...
- `privileged` (Boolean) Specify if privileged permissions
- `read_only_root_file_system` (Boolean) Specify if permission is read only root file system
...
- `use_pvc` (Boolean) Specify if the container needs to use persistent volume claims
