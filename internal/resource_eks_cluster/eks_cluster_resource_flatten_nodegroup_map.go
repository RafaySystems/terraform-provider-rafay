...
func (v *NodeGroupsMapValue) Flatten(ctx context.Context, in *rafay.NodeGroup, state NodeGroupsMapValue) diag.Diagnostics {
	...
	if len(in.Taints) > 0 {
		stTaints := []Taints6Value{}
		if !state.Taints6.IsNull() {
			for _, sT := range state.Taints6.Elements() {
				stTaints = append(stTaints, sT.(Taints6Value))
			}
		}
...
func (v *Iam6Value) Flatten(ctx context.Context, in *rafay.NodeGroupIAM, state Iam6Value) diag.Diagnostics {
	...
	if len(in.AttachPolicyARNs) > 0 {
		...
		v.AttachPolicyArns, d = types.SetValue(types.StringType, aparns)
		diags = append(diags, d...)
	} else {
...
func (v *Taints6Value) Flatten(ctx context.Context, in rafay.NodeGroupTaint, state Taints6Value) diag.Diagnostics {
	var diags diag.Diagnostics
	if in.Key != "" {
