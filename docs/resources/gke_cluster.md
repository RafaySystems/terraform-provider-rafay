...
- `api_version` (String) apiVersion of the resource
...
- `created_by` (Block List, Max: 1) User who created this resource (see [below for nested schema](#nestedblock--metadata--created_by))
- `description` (String) description of the resource
- `display_name` (String) Display Name of the resource
- `labels` (Map of String) labels of the resource
- `modified_by` (Block List, Max: 1) User who last modified this resource (see [below for nested schema](#nestedblock--metadata--modified_by))
...
- `is_sso_user` (Boolean) Whether person is logged in using sso
- `username` (String) Username fo the Person
...
- `authorized_network` (Block List) Add control plane authorized networks to block untrusted non-GCP source IPs from accessing the Kubernetes control plane through HTTPS (see [below for nested schema](#nestedblock--spec--config--network--control_plane_authorized_network--authorized_network))

<a id="nestedblock--spec--config--network--control_plane_authorized_network--authorized_network"></a>
### Nested Schema for `spec.config.network.control_plane_authorized_network.authorized_network`
...
- `disable_http_load_balancing` (Boolean)  The HTTP Load Balancing add-on is required to use the Google Cloud Load Balancer with Kubernetes Ingress. If enabled, a controller will be installed to coordinate applying load balancing configuration changes to your GCP project. Enabled by default.
- `disable_horizontal_pod_autoscaling` (Boolean) Horizontal Pod Autoscaling feature increases or decreases the number of replica pods a replication controller has based on the resource usage of the existing pods. When enabled, it ensures that metrics are collected into Stackdriver Monitoring. Enabled by default.
...
- `config` (Block List, Max: 1) Upgrade settings for the node pool. This single block holds both the surge fields (`max_surge`, `max_unavailable`) and the blue-green fields (`batch_node_count`, `batch_soak_duration`, `node_pool_soak_duration`); which ones take effect depends on `strategy`. (see [below for nested schema](#nestedblock--spec--config--node_pools--upgrade_settings--config))
