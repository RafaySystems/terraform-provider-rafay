...
- `projects` (Block Set) The list of projects this resource is shared with. (see [below for nested schema](#nestedblock--spec--sharing--projects))
...
- `daemon_set_override` (Block List, Max: 1) Allows users to override the default behaviour of DaemonSets for specific nodes by adding additional tolerations. (see [below for nested schema](#nestedblock--spec--system_components_placement--daemon_set_override))
...
- `disable_horizontal_pod_autoscaling` (Boolean) Disables horizontal pod autoscaling (HPA). Horizontal Pod Autoscaling feature increases or decreases the number of replica pods a replication controller has based on the resource usage of the existing pods. When enabled, it ensures that metrics are collected into Stackdriver Monitoring. Enabled by default.
- `disable_http_load_balancing` (Boolean) Disables the HTTP (L7) load balancing controller addon for services. The HTTP Load Balancing add-on is required to use the Google Cloud Load Balancer with Kubernetes Ingress. If enabled, a controller will be installed to coordinate applying load balancing configuration changes to your GCP project. Enabled by default.
...
- `enable_gcs_fuse_csi_driver` (Boolean) Mounts Cloud Storage buckets as local volumes using the GCS Fuse CSI driver. Requires Workload Identity to be enabled.
...
- `config` (Block List, Max: 1) A single, flat configuration block used regardless of `strategy`. Populate `max_surge`/`max_unavailable` when `strategy` is `SURGE`, or `batch_node_count`/`batch_soak_duration`/`node_pool_soak_duration` when `strategy` is `BLUE_GREEN`. (see [below for nested schema](#nestedblock--spec--config--node_pools--upgrade_settings--config))
...
- `batch_node_count` (Number) The absolute number of nodes to drain in a batch. If it is set to zero, this phase will be skipped. Applies when `strategy` is `BLUE_GREEN`.
- `batch_soak_duration` (String) Duration in seconds to wait after each batch finishes draining. This is an opportunity to verify your workload’s health for the batch upgraded. Applies when `strategy` is `BLUE_GREEN`.
- `node_pool_soak_duration` (String) Duration in seconds to wait when all batches are completely drained. This is an opportunity to verify your workload’s health. Applies when `strategy` is `BLUE_GREEN`.
- `max_surge` (Number) The maximum number of nodes that can be created beyond the current size of the node pool during the upgrade process. Applies when `strategy` is `SURGE`.
- `max_unavailable` (Number) The maximum number of nodes that can be simultaneously unavailable during the upgrade process. Applies when `strategy` is `SURGE`.
