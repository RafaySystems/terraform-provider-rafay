...
func (v *NodeGroupsValue) Flatten(ctx context.Context, in *rafay.NodeGroup, state NodeGroupsValue) diag.Diagnostics {
	...
	v.AvailabilityZones = availabilityZones2
	...
	taints := types.SetNull(TaintsValue{}.Type(ctx))
	if len(in.Taints) > 0 {
		stTaints := []TaintsValue{}
		if !state.Taints.IsNull() {
			for _, sT := range state.Taints.Elements() {
				stTaints = append(stTaints, sT.(TaintsValue))
			}
		}
		...
		taints, d = types.SetValue(TaintsValue{}.Type(ctx), taintsList)
		diags = append(diags, d...)
	}
	v.Taints = taints
...
func (v *TaintsValue) Flatten(ctx context.Context, in rafay.NodeGroupTaint, state TaintsValue) diag.Diagnostics {
	var diags diag.Diagnostics
...
func (v *IamValue) Flatten(ctx context.Context, in *rafay.NodeGroupIAM, state IamValue) diag.Diagnostics {
	...
	if in.AttachPolicy != nil {
		if isPolicyV1 && !isPolicyV2 {
			...
			diags = append(diags, d...)
			v.AttachPolicy, d = types.SetValue(AttachPolicyValue{}.Type(ctx), []attr.Value{attachPolicy})
			diags = append(diags, d...)
			...
			v.AttachPolicy = types.SetNull(AttachPolicyValue{}.Type(ctx))
		}
	} else {
		v.AttachPolicy = types.SetNull(AttachPolicyValue{}.Type(ctx))
		v.AttachPolicyV2 = types.StringNull()
...
func (v *AttachPolicyValue) Flatten(ctx context.Context, in *rafay.InlineDocument) diag.Diagnostics {
	...
	statement := types.SetNull(StatementValue{}.Type(ctx))
	if len(in.Statement) > 0 {
		...
		statement, d = types.SetValue(StatementValue{}.Type(ctx), stms)
		diags = append(diags, d...)
	}
