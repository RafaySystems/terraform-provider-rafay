...
func (v *SpecValue) Flatten(ctx context.Context, in *rafay.EKSSpec, state SpecValue) diag.Diagnostics {
	...
	if !state.SystemComponentsPlacement.IsNull() {
		scpList := make([]SystemComponentsPlacementValue, len(state.SystemComponentsPlacement.Elements()))
		d = state.SystemComponentsPlacement.ElementsAs(ctx, &scpList, false)
		diags = append(diags, d...)
	...
	if in.SystemComponentsPlacement != nil {
		...
		d = scp.Flatten(ctx, in.SystemComponentsPlacement, stScp)
		diags = append(diags, d...)
...
func (v *ClusterConfigValue) Flatten(ctx context.Context, in rafay.EKSClusterConfig, state ClusterConfigValue) diag.Diagnostics {
	...
	if len(in.Addons) > 0 {
		...
		addons, d = types.SetValue(AddonsValue{}.Type(ctx), addonsList)
		diags = append(diags, d...)
	}
	v.Addons = addons
	...
	if len(in.ManagedNodeGroups) > 0 {
		if isManagedNodeGroupsMap {
			...
			if !state.ManagedNodegroups.IsNull() {
	...
	if len(in.FargateProfiles) > 0 {
		...
		for _, fargateProfile := range in.FargateProfiles {
			...
			diags = append(diags, d...)
			fargateProfilesList = append(fargateProfilesList, fp)
		}
	...
	stCloudWatch := make([]CloudWatchValue, 0, len(state.CloudWatch.Elements()))
	if !state.CloudWatch.IsNull() {
		d = state.CloudWatch.ElementsAs(ctx, &stCloudWatch, false)
		diags = append(diags, d...)
	}

	stCW := CloudWatchValue{}
	if len(stCloudWatch) > 0 {
		stCW = stCloudWatch[0]
	}

	if in.CloudWatch != nil {
		cloudWatch := NewCloudWatchValueNull()
		d = cloudWatch.Flatten(ctx, in.CloudWatch, stCW)
		diags = append(diags, d...)
	...
	// hack: API sends empty struct for IdentityMappings even it is set to nil. This is to avoid unnecessary diffs.
	if in.IdentityMappings != nil && (len(in.IdentityMappings.Arns) > 0 || len(in.IdentityMappings.Accounts) > 0) {
...
func (v *CloudWatchValue) Flatten(ctx context.Context, in *rafay.EKSClusterCloudWatch, state CloudWatchValue) diag.Diagnostics {
	var diags, d diag.Diagnostics
	if in == nil {
		return diags
	}

	stClusterLogging := make([]ClusterLoggingValue, 0, len(state.ClusterLogging.Elements()))
	if !state.ClusterLogging.IsNull() {
		d = state.ClusterLogging.ElementsAs(ctx, &stClusterLogging, false)
		diags = append(diags, d...)
	}

	stCL := ClusterLoggingValue{}
	if len(stClusterLogging) > 0 {
		stCL = stClusterLogging[0]
	}

	// hack: API sends empty struct for ClusterLogging even it is set to nil. This is to avoid unnecessary diffs.
	if in.ClusterLogging != nil && !reflect.DeepEqual(in.ClusterLogging, &rafay.EKSClusterCloudWatchLogging{}) {
		clusterLogging := NewClusterLoggingValueNull()
		d = clusterLogging.Flatten(ctx, in.ClusterLogging, stCL)
		diags = append(diags, d...)
...
func (v *ClusterLoggingValue) Flatten(ctx context.Context, in *rafay.EKSClusterCloudWatchLogging, state ClusterLoggingValue) diag.Diagnostics {
	var diags, d diag.Diagnostics
	if in == nil {
		return diags
	}
	...
	if in.LogRetentionInDays != 0 {
		...
		// hack: API can not differenciate nil and zero value of log retention days. This is to avoid unnecessary diffs.
		if !state.IsNull() && !state.LogRetentionInDays.IsNull() {
			v.LogRetentionInDays = state.LogRetentionInDays
		} else {
			v.LogRetentionInDays = types.Int64Null()
		}
	}
...
func (v *AddonsValue) Flatten(ctx context.Context, in *rafay.Addon, state AddonsValue) diag.Diagnostics {
	...
	attachPolicyArns3 := types.SetNull(types.StringType)
	if len(in.AttachPolicyARNs) > 0 {
		...
		attachPolicyArns3, d = types.SetValue(types.StringType, policyARNs)
		diags = append(diags, d...)
	}
	...
	if in.AttachPolicy != nil {
		if isPolicyV1 && !isPolicyV2 {
			...
			diags = append(diags, d...)
			v.AttachPolicy3, d = types.SetValue(AttachPolicy3Value{}.Type(ctx), []attr.Value{attachPolicy})
			diags = append(diags, d...)
			...
			v.AttachPolicy3 = types.SetNull(AttachPolicy3Value{}.Type(ctx))
		}
	} else {
		v.AttachPolicy3 = types.SetNull(AttachPolicy3Value{}.Type(ctx))
		v.AttachPolicyV22 = types.StringNull()
	...
	if in.ConfigurationValues != "" {
		// hack: For Windows node groups, AWS automatically includes `enableWindowsIpam = "true"` for vpc-cni addon.
		// This is to avoid unnecessary diffs.
		if in.Name == "vpc-cni" && strings.Contains(in.ConfigurationValues, "enableWindowsIpam") {
			if state.IsNull() || state.ConfigurationValues.IsNull() ||
				!strings.Contains(state.ConfigurationValues.String(), "enableWindowsIpam") {

				// remove "enableWindowsIpam" from in.ConfigurationValues JSON string
				var configMap map[string]any
				var json2 = jsoniter.ConfigCompatibleWithStandardLibrary
				err := json2.Unmarshal([]byte(in.ConfigurationValues), &configMap)
				if err != nil {
					diags.AddError("Configuration Values Unmarshal Error", err.Error())
				} else {
					delete(configMap, "enableWindowsIpam")
					updatedConfigBytes, err := json2.Marshal(configMap)
					if err != nil {
						diags.AddError("Configuration Values Marshal Error", err.Error())
					} else {
						in.ConfigurationValues = string(updatedConfigBytes)
					}
				}

				// set configurationvalues to empty string if no other keys present
				if len(configMap) == 0 {
	...
	if in.WellKnownPolicies != nil {
		...
		diags = append(diags, d...)
		v.WellKnownPolicies3, d = types.SetValue(WellKnownPolicies3Value{}.Type(ctx), []attr.Value{policies})
		diags = append(diags, d...)
	} else {
		v.WellKnownPolicies3 = types.SetNull(WellKnownPolicies3Value{}.Type(ctx))
	}

	podIdentityAssociations2 := types.SetNull(PodIdentityAssociations2Value{}.Type(ctx))
	if len(in.PodIdentityAssociations) > 0 {
		...
		podIdentityAssociations2, d = types.SetValue(PodIdentityAssociations2Value{}.Type(ctx), pias)
		diags = append(diags, d...)
	}
...
func (v *PodIdentityAssociations2Value) Flatten(ctx context.Context, in *rafay.IAMPodIdentityAssociation) diag.Diagnostics {
	...
	permissionPolicyArns := types.SetNull(types.StringType)
	if len(in.PermissionPolicyARNs) > 0 {
		arns := []attr.Value{}
		for _, arn := range in.PermissionPolicyARNs {
			arns = append(arns, types.StringValue(arn))
		}
		permissionPolicyArns, d = types.SetValue(types.StringType, arns)
		diags = append(diags, d...)
	}
	...
	if in.WellKnownPolicies != nil {
		...
		d = policies.Flatten(ctx, in.WellKnownPolicies)
		diags = append(diags, d...)
		v.WellKnownPolicies4, d = types.SetValue(WellKnownPolicies4Value{}.Type(ctx), []attr.Value{policies})
		diags = append(diags, d...)
	} else {
		v.WellKnownPolicies4 = types.SetNull(WellKnownPolicies4Value{}.Type(ctx))
	}
...
func (v *SystemComponentsPlacementValue) Flatten(ctx context.Context, in *rafay.SystemComponentsPlacement, state SystemComponentsPlacementValue) diag.Diagnostics {
	var diags, d diag.Diagnostics
	if in == nil {
		return diags
	}
	...
	if in.DaemonsetOverride != nil {
		...
		d = daemonsetOverride.Flatten(ctx, in.DaemonsetOverride, stDso)
		diags = append(diags, d...)
...
func (v *DaemonsetOverrideValue) Flatten(ctx context.Context, in *rafay.DaemonsetOverride, state DaemonsetOverrideValue) diag.Diagnostics {
	var diags, d diag.Diagnostics
	if in == nil {
		return diags
	}

	if in.NodeSelectionEnabled != nil {
		v.NodeSelectionEnabled = types.BoolPointerValue(in.NodeSelectionEnabled)
	} else {
		// hack: API sends nil for NodeSelectionEnabled even it is set to false. This is to avoid unnecessary diffs.
		if !state.IsNull() && !state.NodeSelectionEnabled.IsNull() && !state.NodeSelectionEnabled.ValueBool() {
			v.NodeSelectionEnabled = state.NodeSelectionEnabled
		} else {
			v.NodeSelectionEnabled = types.BoolNull()
		}
	}
